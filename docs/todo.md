...
- [x] Built the chatbot **priming/context seam + all three entry points** — per-listing "Ask about this car", compare-drawer "Ask AI", and the plain-English homepage search — each chosen via a 3-agent solution contest.
...
- **Fuel-economy data gap:** there is no economy/L-100km field on listings, so Rebi honestly declines running-cost questions. If dealers want it answered, add an economy field to the listing schema.
