...
| Parts enquiry (parts-desk/lead) | STUB_PARTS | PARTS_API_KEY | .dev.vars / wrangler secret | real parts enquiries posted to the dealer's parts-desk/lead system (`/parts` → `/api/parts-enquiry`, stub in `src/stubs/parts-enquiry.ts`); also gated by `parts.enabled`. Deterministic stub returns a `PARTS-<hash>` reference, quotes no price and guarantees no fitment | ~1 day (API contract + mapping) |
| Fleet enquiry (fleet CRM/lead) | STUB_FLEET | FLEET_CRM_API_KEY | .dev.vars / wrangler secret | real fleet & business enquiries posted to the dealer's fleet CRM/lead system (`/fleet` → `/api/fleet-enquiry`, stub in `src/stubs/fleet-enquiry.ts`); also gated by `fleet.enabled`. Deterministic stub returns a `FLEET-<hash>` reference, quotes no price and commits no terms | ~1 day (API contract + mapping) |
| Contact enquiry (front-desk lead/CRM) | STUB_CONTACT | CONTACT_API_KEY | .dev.vars / wrangler secret | real general enquiries posted to the dealer's lead/CRM system (`/contact` → `/api/contact-enquiry`, stub in `src/stubs/contact-enquiry.ts`); also gated by `contact.enabled`. Deterministic stub returns a `MSG-<hash>` reference and invents no contact fact (page facts come from the businessInfo doc) | ~1 day (API contract + mapping) |
| Careers register-interest (lead/ATS) | STUB_CAREERS | CAREERS_API_KEY | .dev.vars / wrangler secret | real register-interest submissions posted to the dealer's lead/ATS system (`/careers` → `/api/careers-enquiry`, stub in `src/stubs/careers-enquiry.ts`); also gated by `careers.enabled`. Deterministic stub returns a `CAR-<hash>` reference; an expression of interest, not a job application | ~1 day (API contract + mapping) |
